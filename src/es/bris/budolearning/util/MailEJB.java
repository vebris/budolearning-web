...
public class MailEJB implements MailEJBLocal {
	...
	public void enviarMailUsuarioCreacion(String mail, Usuario usuario) {
		...
		try {
			...
			String sms = "¡Bienvenido!<br/><br/>"
					...
					+ "<b>Nombre de usuario:</b> " + usuario.getLogin()
					+ "<br/>" + "<b>URL</b>: http://budolearning.noip.me"
	...
	@Override
	public void enviarMailUsuarioRegistro(String mail, Usuario usuario,
			Usuario profesor) {
		MimeMessage message = new MimeMessage(mailSession);
		try {
			InternetAddress[] address = { new InternetAddress(mail) };
			...
			String nombreProfesor = "ADMINISTRADOR";
			if (usuario.getEntrena() != null)
				nombreProfesor = usuario.getEntrena().getNombre();

			String sms = "¡Bienvenido!<br/><br/>"
					+ "Gracias por unirse a nosotros. <br/><br/>"
					+ "Falta la validación del profesor asociado al Club ("
					+ nombreProfesor
					+ ") "
			...
			sms = "Buenos días " + profesor.getNombre() + ",<br/><br/>"
					+ "Has de validar al usuario " + usuario.getLogin() + " ("
	...
	public int enviarMailCurso(Curso curso, List<Usuario> usuarios) {
		...
		try {
			...
			message.setSubject("[BudoLearning] Curso de "
					+ curso.getDisciplina().getNombre());
			message.setSentDate(new java.util.Date());
			SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm");
			String sms = "Buenos días,<br/><br/>" + " Nombre del Curso: "
					+ curso.getNombre() + " Descripcion: "
					+ curso.getDescripcion() + " Profesor/es: "
					+ curso.getProfesor() + " Duración : "
					+ sdf.format(curso.getInicio()) + " a "
					+ sdf.format(curso.getFin()) + " Lugar: "
					+ curso.getDireccion() + " Precio " + curso.getPrecios()
					+ "<br/><br/>Saludos.<br/>BUDOLEARNING";
			message.setContent(sms, "text/html");
			Transport.send(message);
	...
	public void enviarMailNuevoFichero(Fichero fichero, Usuario administrador) {
		MimeMessage message = new MimeMessage(mailSession);
		try {
			InternetAddress[] address = { new InternetAddress(administrador.getMail()) };
			message.setRecipients(Message.RecipientType.TO, address);
			message.setSubject("[BudoLearning] Nuevo Fichero");
			message.setSentDate(new java.util.Date());
			
			String sms = "Nuevo fichero subido por " 
					+ fichero.getUsuario().getNombre() + " " + fichero.getUsuario().getApellido1() + " " + fichero.getUsuario().getApellido2() + "<br/>"
					+ "Debe realizar la asignación del fichero al recurso correspondiente."
					+ "<br/><br/>Saludos.<br/>BUDOLEARNING";
			message.setContent(sms, "text/html");
			Transport.send(message);
	...
	public void enviarMailNuevoFicheroAceptado(Fichero fichero, Usuario administrador) {
		...
		try {
			InternetAddress[] address = { new InternetAddress(administrador.getMail()) };
			message.setRecipients(Message.RecipientType.TO, address);
			message.setSubject("[BudoLearning] Nuevo Fichero Aceptado");
			message.setSentDate(new java.util.Date());
			
			String sms = "Buenos días, <br/><br/>"
					+ "Gracias por el apoyo y haber subido un al sistema.<br/>" 
					+ "Este fichero ha sido asignado al recurso "+ fichero.getRecurso().getNombre() +".<br/>"
					+ "<br/><br/>Saludos y muchas gracias.<br/>BUDOLEARNING";
	...
	public void enviarMailNuevoFicheroRechazado(Fichero fichero, Usuario administrador) {
		...
		try {
			InternetAddress[] address = { new InternetAddress(administrador.getMail()) };
			message.setRecipients(Message.RecipientType.TO, address);
			message.setSubject("[BudoLearning] Nuevo Fichero Rechazado");
			message.setSentDate(new java.util.Date());
			
			String sms = "Buenos días, <br/><br/>"
					+ "Gracias por el apoyo y haber subido un al sistema.<br/>" 
					+ "Este fichero no ha podido asignarse a ningún recurso por alguna de las siguientes causas:<br/>"
					+ "<ul>"
					+ "   <li>El fichero tiene derechos de autor no confirmados.</li>"
					+ "   <li>El fichero no se ajusta a ningún recurso actual.</li>"
					+ "</ul>"
					+ "<br/>Si tiene cualquier consulta no dude en ponerse en contacto con nosotros." 
					+ "<br/><br/>Saludos y muchas gracias.<br/>BUDOLEARNING";
